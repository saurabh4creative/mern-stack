        ...
        const getData = async () => {
            try{   
                ...
                if( userResponse.status ){
                    const karbonDetail = await axios.get(`${BASE_URL}/api/v1/users/karbon/${id}`);
                    if( karbonDetail.data.status ){
                        setValue('name', karbonDetail.data.data.name )
                        setValue('discription', karbonDetail.data.data.discription )
                        setValue('start', karbonDetail.data.data.start )
                        setValue('end', karbonDetail.data.data.end ) 
                        setValue('user', karbonDetail.data.data.listUser.map((item)=>item._id)) 
                    }
                }
            }catch(err){
